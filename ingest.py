"""Unified ingestion: All markets from all sources."""
...
def fetch_odds_api_games(session, api_key, config):
    ...
    for sport in config["sports"]:
        for mkt in config["markets"]:
            ...
            params = {"apiKey": api_key, "regions": ",".join(config["regions"]), "markets": mkt, "oddsFormat": "decimal", "dateFormat": "iso"}
            resp = session.get(url, params=params, timeout=20)
            data = resp.json() if resp.status_code == 200 else []
            print(f"  {sport}/{mkt}: {len(data)} games")
            time.sleep(config.get("polling", {}).get("request_delay_seconds", 0))
                    ...
                    for m in book.get("markets", []):
                        ...
                        for out in m.get("outcomes", []):
                            ...
                            if m["key"] == "totals":
                                ...
                                if o == h or h in o: side = "home"
                                elif o == a or a in o: side = "away"
                                elif o in {"draw", "tie", "x"}: side = "draw"
                                else: continue
...
def fetch_odds_api_futures(session, api_key):
    """Fetch futures from Odds API."""
    games, rows = {}, []
    now = utc_now_iso()
                ...
                for mkt in book.get("markets", []):
                    ...
                    for i, out in enumerate(outcomes):
                        ...
                        rows.append({
                            ...
                            "source_event_id": None, "source_market_id": None, "outcome": team,
                        })
...
# ============================================================
# POLYMARKET - Futures only (no game markets currently)
# ============================================================

def fetch_polymarket(session):
    """Fetch Polymarket futures."""
    games, rows = {}, []
    now = utc_now_iso()
    ...
    # Match to our futures
    FUTURES = {
        "futures_basketball_nba_championship_winner": "nba finals",
        "futures_icehockey_nhl_championship_winner": "stanley cup",
    }
    
    for futures_id, phrase in FUTURES.items():
        teams = []
...
def fetch_kalshi(session):
    """Extract game data from Kalshi parlay markets."""
    games, rows = {}, []
    now = utc_now_iso()
    ...
    for m in all_markets:
        for leg in m.get('mve_selected_legs', []):
            ...
            rows.append({"game_id": game_id, "market": "h2h", "side": normalize_team(selection),
                        "line": 0.0, "source": "kalshi", "provider": "kalshi", "price": None,
...
def ingest():
    ...
    try:
        with requests.Session() as session:
            ...
            if api_key:
                ...
                all_games.update(g); all_rows.extend(r)
            ...
            print("\n[ODDS API - Futures]")
            if api_key:
                g, r = fetch_odds_api_futures(session, api_key)
                all_games.update(g); all_rows.extend(r)
        ...
        # Save everything
        upsert_rows(conn, "games", ["game_id"], ["league", "commence_time", "home_team", "away_team", "last_refreshed"], all_games.values())
        upsert_rows(conn, "market_latest", ["game_id", "market", "side", "line", "source", "provider"],
                    ["price", "implied_prob", "devigged_prob", "provider_updated_at", "last_refreshed", "source_event_id", "source_market_id", "outcome"], all_rows)
    ...
    print(f"\n✅ TOTAL: {len(all_games)} markets, {len(all_rows)} rows")
